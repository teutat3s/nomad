	...
	if err := retryMax(maxSysBatchScheduleAttempts, s.process, progress); err != nil {
		if statusErr, ok := err.(*SetStatusError); ok {
			return setStatus(
				...
				s.planner,
				s.eval,
				s.nextEval,           // next eval
				nil,                  // spawned block
				s.failedTGAllocs,     // tg metrics
				statusErr.EvalStatus, // status
				err.Error(),          // description
				s.queuedAllocs,       // queued allocs
				"",                   // deployment id
			)
		}
		return err
...
func (s *SysBatchScheduler) computePlacements(place []allocTuple) error {
